...
public class FormHierarchyActivity extends CollectAbstractActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (formController == null) {
            ...
            CollectInitialiser.INSTANCE.logNullFormControllerEvent("FormHierarchyActivity");
            return;
        }
    ...
    public void onElementClick(HierarchyElement element) {
        ...
        switch (element.getType()) {
            ...
            case REPEAT_INSTANCE:
                // Hide the picker.
                repeatGroupPickerIndex = null;
                CollectInitialiser.INSTANCE.getFormController().jumpToIndex(index);
                setResult(RESULT_OK);
                refreshView();
                break;
        }
    ...
    void onQuestionClicked(FormIndex index) {
        CollectInitialiser.INSTANCE.getFormController().jumpToIndex(index);
        if (CollectInitialiser.INSTANCE.getFormController().indexIsInFieldList()) {
            try {
                CollectInitialiser.INSTANCE.getFormController().stepToPreviousScreenEvent();
            } catch (JavaRosaException e) {
                Timber.d(e);
                createErrorDialog(e.getCause().getMessage());
                return;
